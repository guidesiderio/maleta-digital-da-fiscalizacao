...
- **vercel.json** — Vercel deployment configuration with security headers (CSP, X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy, X-XSS-Protection).
...
- **Hosting**: Deployed on Vercel (Hobby plan), auto-deploys from `main` branch. Production URL: `maleta-digital-da-fiscalizacao.vercel.app`. Security headers configured via `vercel.json`. Also compatible with GitHub Pages (static files, no build step).
